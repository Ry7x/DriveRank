  ...
  useEffect(() => {
    ...
    try {
      if (userLocation) {
        ...
        if (bounds.isValid()) {
          map.fitBounds(bounds, { padding: [30, 30], animate: true, duration: 2.0 });
        }
      }
    } catch (e) {}
...
export default function RouteMap({ points, userLocation, heading, color = 'accent' }: RouteMapProps) {
        ...
        html: `
          <div class="lead-arrow-container relative w-8 h-8 flex items-center justify-center">
            ...
            <svg width="24" height="24" viewBox="0 0 40 40" fill="none">
              <path d="M20 4L34 34L20 27L6 34L20 4Z" fill="${color === 'red' ? '#ef4444' : '#4de0f4'}" stroke="white" stroke-width="2" stroke-linejoin="round"/>
            </svg>
          </div>
        `,
        iconSize: [32, 32],
  ...
  if (!isClient) return <div className="w-full h-full bg-black rounded-2xl" />;

  const startPoint = points[0];
  const endPoint = points[points.length - 1];
  const lastAnimatedPoint = animatedPoints[animatedPoints.length - 1];

  const mainColor = color === 'red' ? '#ef4444' : '#4de0f4';
  const glowColor = color === 'red' ? '#7f1d1d' : '#06b6d4';
  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-black rounded-2xl border border-white/5 shadow-inner">
      <MapContainer 
        ...
        zoom={17} 
        style={{ height: '100%', width: '100%', zIndex: 1, background: 'black' }}
        zoomControl={false}
        ...
        {animatedPoints.length > 1 && (
          <>
            <Polyline 
              ...
              pathOptions={{ color: glowColor, weight: 12, opacity: 0.15, lineJoin: 'round' }} 
            />
            <Polyline 
              positions={animatedPoints} 
              pathOptions={{ color: mainColor, weight: 6, opacity: 1, lineJoin: 'round' }} 
            />
          </>
        )}
      ...
      <div className="absolute top-2 right-2 z-[400] pointer-events-none apple-slide-up">
        <Badge className="bg-black/60 backdrop-blur-md border-white/10 text-[8px] font-black uppercase tracking-widest py-1 px-2 flex items-center gap-1 shadow-xl">
          <Zap className={cn("w-2 h-2", color === 'red' ? "text-red-500" : "text-accent")} /> {userLocation ? 'LIVE TELEMETRY' : 'RECAP DATA'}
        </Badge>
      </div>
    </div>
  );
