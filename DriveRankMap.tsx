...
import { Gauge, Loader2, Mail, Lock, LogIn, Car, Palette, Users } from "lucide-react";
...
export default function Home() {
  ...
  const initializeUserProfile = async (user: User, details?: any, isNew: boolean = false) => {
    ...
    try {
      ...
      if (!userDoc.exists()) {
        await setDoc(userDocRef, {
          ...
          community: details?.community || 'Tjetër',
          ...
          modifications: 'Stock',
          profilePhotoUrl: "",
          profileIcon: isAdmin ? "crown" : "speedometer",
          coverPhotoUrl: "",
          followedUserIds: [],
      ...
      } else if (isAdmin && !userDoc.data().isAdmin) {
        await updateDoc(userDocRef, { isAdmin: true, isPremium: true });
      }
    } catch (e) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isRegistering) {
        if (!username || !city || !community || !carBrand || !carModel) {
        ...
        await Promise.all([
          ...
          initializeUserProfile(result.user, { username, city, community, carBrand, carModel, carColor }, true)
        ]);

        toast({ title: "MIRËSEERDHËT NË ELITË!", description: "Llogaria juaj u krijua me sukses." });
      } else {
        const result = await signInWithEmailAndPassword(auth, email, password);
        await initializeUserProfile(result.user);
        toast({ title: "MIRË SE U KTHEVE!", description: "Vazhdo rrugëtimin drejt shpejtësisë." });
      }
  ...
  return (
    <div className="flex flex-col min-h-screen overflow-y-auto p-6 bg-background safe-top safe-bottom no-scrollbar">
      <div className="mt-8 mb-8 space-y-4 apple-slide-up">
      ...
      <div className="glass border-white/5 rounded-[2rem] p-6 shadow-2xl space-y-6 mb-8 apple-slide-up stagger-1">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          {isRegistering && (
            <div className="grid gap-4">
              <div className="space-y-1.5">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Emri i Shoferit/es</Label>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={city} onValueChange={setCity} required>
                  ...
                  <SelectContent className="glass border-white/10 max-h-[240px]">
                    {KOSOVO_CITIES.map(c => (
                      <SelectItem key={c} value={c} className="font-bold text-[11px] uppercase">{c}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-1.5">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Komuniteti Juaj</Label>
                <div className="relative">
                  <Input 
                    placeholder="p.sh. Marakli t'kerreve" 
                    value={community} 
          ...
          <Button type="submit" disabled={loading} className="w-full h-14 rounded-2xl bg-accent hover:bg-accent/80 text-background font-black uppercase text-base shadow-xl neon-glow mt-2 active:scale-95 transition-transform">
